...
async function handleGetUsage(request, env) {
  ...
  return jsonResponse({
      usage: {
          requests_today: user.usage.requests_today,
          tokens_today: user.usage.tokens_today,
          total_requests: user.usage.total_requests,
          total_tokens: user.usage.total_tokens
      },
      limits: {
      ...
      remaining: {
          requests: Math.max(0, tierLimits.requests_per_day - user.usage.requests_today),
          tokens: Math.max(0, tierLimits.tokens_per_day - user.usage.tokens_today)
      }
  });
}
